...
def is_english_line(line):
    ...
    return (
        re.match(r"^\d+:\d+", stripped)  # verse line like 16:5
        or re.match(r"^[a-z]\.", stripped)  # a., b., etc.
    )
...
def clean_and_group(input_path, output_path):
    ...
    with open(input_path, "r", encoding="utf-8") as infile:
        for raw_line in infile:
            line = raw_line.strip()  # remove all leading/trailing whitespace including indentation
            ...
            if subline_match and current_verse:
                verses[current_verse].append(f"{subline_match.group(1)} {subline_match.group(2)}")

    # Write to output with blank line between verses
    with open(output_path, "w", encoding="utf-8") as outfile:
        for verse_num in sorted(verses, key=lambda x: (int(x.split(":")[0]), int(x.split(":")[1]))):
